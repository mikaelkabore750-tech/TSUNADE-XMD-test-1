...
function canUseCommand(m, tsunade, isAdmins) {
    const botNumber = tsunade.user?.id?.split(":")[0] || "";
...
cmd({
    ...
    try {
        if (!canUseCommand(m, tsunade, false)) return reply("⚠️ Only bot or admins can use this command!");
        if (!m.quoted) return reply("⚠️ Reply to the user you want to block!");

        const target = m.quoted.sender;
        await tsunade.updateBlockStatus(target, "block");
...
cmd({
    ...
    category: "main",
    filename: __filename
}, async (tsunade, mek, m, { from, isGroup, isAdmins, isBotAdmins, reply }) => {
    try {
        if (!isGroup) return reply("⚠️ Group only!");
        if (!canUseCommand(m, tsunade, isAdmins)) return reply("⚠️ Only bot or admins can use this command!");
        if (!isBotAdmins) return reply("⚠️ Bot must be admin!");
        if (!m.quoted) return reply("⚠️ Reply to the user you want to kick!");

        const target = m.quoted.sender;
        await tsunade.groupParticipantsUpdate(from, [target], "remove");
...
cmd({
    ...
    try {
        ...
        const groupMetadata = await tsunade.groupMetadata(from);
        const members = groupMetadata.participants;
        const targets = members.filter(p => !p.admin).map(p => p.id);

        for (let user of targets) {
            await tsunade.groupParticipantsUpdate(from, [user], "remove");
        }
...
cmd({
    ...
    category: "main",
    filename: __filename
}, async (tsunade, mek, m, { from, isGroup, isAdmins, reply }) => {
    try {
        if (!isGroup) return reply("⚠️ Group only!");
        ...
        return tsunade.sendMessage(from, { text, mentions: members.map(u => u.id) });
    } catch (e) {
...
cmd({
    ...
    try {
        ...
        await tsunade.groupSettingUpdate(from, "announcement");
...
cmd({
    ...
    try {
        ...
        await tsunade.groupSettingUpdate(from, "not_announcement");
...
cmd({
    ...
    try {
        ...
        const target = m.quoted.sender;
        await tsunade.groupParticipantsUpdate(from, [target], "promote");
...
cmd({
    ...
    try {
        ...
        const target = m.quoted.sender;
        await tsunade.groupParticipantsUpdate(from, [target], "demote");
