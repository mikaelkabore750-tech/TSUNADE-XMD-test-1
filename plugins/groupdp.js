...
cmd(
  ...
  async (tsunade, mek, m, { from, reply }) => {
    try {
      if (!m.isGroup) return reply("❌ This command works only in groups.");

      const groupMeta = await tsunade.groupMetadata(from);
      ...
      let icon;
      try {
        icon = await tsunade.profilePictureUrl(from, "image");
      } catch {
        icon = null;
      }
  ...
  async (tsunade, mek, m, { from, reply }) => {
    try {
      ...
      for (const member of groupMeta.participants) {
        try {
          const dpUrl = await tsunade.profilePictureUrl(member.id, "image");
          if (dpUrl) {
